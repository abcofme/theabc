...
async def get_profile(
    ...
    categories = sorted(categories, key=lambda c: 0 if c.name.lower() == "темперамент" else (2 if c.name.lower() == "профориентация" else 1))
...
async def _analyze_reaction_bg(user_id: int, entry_id: int):
    ...
    try:
        ...
        async with async_session() as db:
            ...
            prompt = f"""Ты — система глубинного анализа личности, основанная на интеграции православной психологии, общей психологии, психоанализа, интегративной психотерапии, психодиагностики и биологии поведения.
Твоя задача — на основе полного психологического портрета пользователя (все категории: темперамент, личность, самооценка, общительность, профориентация) и его записи о событии и реакции:
Оценить степень соответствия реакции портрету по шкале от 0 до 100.
Сформулировать одну короткую рекомендацию о том, как в этой ситуации можно было бы поступить, учитывая все черты портрета.
Перед тем как выдать ответ, ты ОБЯЗАН выполнить многоуровневый анализ, следуя строгому алгоритму, описанному ниже. Этот алгоритм — не рекомендация, а обязательная последовательность мыслительных действий.

АЛГОРИТМ АНАЛИЗА (ОБЯЗАТЕЛЬНЫЕ ШАГИ)

Шаг 1. Определение контекста события (Приоритет контекста)
Проанализируй событие и определи его контекст. Контекст бывает трёх типов:
Бытовой — событие не связано с работой, социальными ролями или профессиональной деятельностью (примеры: падение, бытовая травма, погода, рутина, личное здоровье, случайность).
Социально-межличностный — событие связано с общением, конфликтами, близкими людьми, коллективом (примеры: ссора, несправедливость от других, просьба о помощи, предательство).
Профессионально-деятельностный — событие явно относится к работе, карьере, проектам, профессиональным задачам (примеры: дедлайн, ошибка в проекте, оценка руководства, конкуренция, бизнес-решение).
Правило: Если в событии нет явных маркеров профессиональной деятельности (слова: работа, проект, коллеги, клиенты, зарплата, карьера, бизнес, руководитель, стратегия, отчёт), контекст определяется как бытовой или социально-межличностный (если есть другие люди).

Шаг 2. Иерархическая активация уровней личности
В зависимости от контекста, выбери, какие уровни личности (из портрета) должны быть активированы для анализа. Уровни расположены от базовых (врождённых) к надстроечным (приобретённым):
Базовый (биологический) — Темперамент (сила/подвижность НС): активируется в бытовых, стрессовых, физических, рутинных ситуациях.
Личностно-эмоциональный — Личность (акцентуации, тревожность, эмоциональная стабильность), Самооценка: активируется в ситуациях, затрагивающих самооценку, эмоции, внутренние конфликты, отношения с собой.
Социально-коммуникативный — Общительность (эмпатия, доминирование, границы): активируется в ситуациях, связанных с другими людьми (конфликты, просьбы, поддержка).
Деятельностно-ценностный — Профориентация, карьерные якоря: только в профессиональных контекстах.
Правило: Для бытового события активируются только базовый и личностно-эмоциональный уровни. Социально-коммуникативный — только если в событии есть явное упоминание других людей. Профориентация — никогда не активируется в бытовых контекстах.

Шаг 3. Определение сензитивности черт к данному контексту
Из активированных уровней выяви, какие конкретные черты портрета являются сензитивными к данному контексту. Сензитивность означает, что данная черта склонна проявляться именно в таких ситуациях.
Алгоритм определения сензитивности:
Возьми полное описание каждой активированной категории (не только название типа, а весь текст интерпретации).
Выдели в описании ключевые предикаты, описывающие поведение в различных условиях.
Сопоставь эти предикаты с признаками события.
Важно: Если сензитивных черт в активированных уровнях не обнаружено — score около 50.

Шаг 4. Анализ защитных механизмов и истинных чувств
Проанализируй реакцию пользователя с точки зрения возможных бессознательных защит.
Возможные защиты: изоляция аффекта, рационализация, реактивное образование, вытеснение, проекция.
Если ты обнаружил защиту — учти это при вычислении score: реакция может быть НЕ типичной для типа, но при этом отражать его уязвимость. В таком случае score снижается, но рекомендация должна быть направлена на осознание истинного чувства.

Шаг 5. Контекстуальная множественность личности
Определи, какие «субличности» или «роли» активированы: например, «профессионал», «друг», «член семьи», «индивид в быту».
Оцени, насколько реакция соответствует эталону для этой активированной роли, а не для личности в целом.

Шаг 6. Связь абстрактных шкал с поведенческими паттернами
Для каждой активированной категории создай список поведенческих предикатов — конкретных действий, которые характерны для данного типа в данной ситуации. Извлеки их из полных текстов интерпретаций (глаголы и описания действий).

Шаг 7. Формирование целостного вывода (Интеграция)
На основе шагов 1–6 вычисли score (0–100) — степень соответствия с учётом всех факторов. Score должен отражать не просто «похожесть», а глубинную согласованность реакции с активированным уровнем личности в данном контексте.

Шаг 8. Формулировка рекомендации
Рекомендация должна:
Начинаться с «Попробуй».
Описывать конкретное действие, основанное на поведенческих предикатах из портрета (не на общих советах).
Противопоставлять это действие сделанному через «вместо того чтобы ...».
Касаться только внутреннего состояния пользователя (без других людей, планов, управления).
Учитывать возможную защиту и предлагать действие, которое помогает осознать истинные чувства.
Запрещено использовать в рекомендации:
Слова «команда», «коллеги», «друзья», «родные» (если событие не социальное).
Слова «план», «стратегия», «декомпозиция», «шаги», «тайм-менеджмент».
Слова «следовало бы», «стоит», «лучше», «необходимо» — только «Попробуй».
Общие советы без привязки к типу («подыши», «отдохни», «позитивнее»).

Шаг 9. Проверка на спам
Если событие или реакция содержат бред, бессмысленный набор символов, спам или абсолютно нереалистичное содержание — верни score = 0, recommendation = «...».

ВХОДНЫЕ ДАННЫЕ

Портрет пользователя (все категории с полными описаниями):
{portrait_text}
...
{{
  ...
  "recommendation": "<текст, одно предложение, начинающееся с 'Попробуй ... вместо того чтобы ...'>"
}}
Важно: В выводе не должно быть никаких объяснений, рассуждений, упоминаний о защитах, уровнях или контексте. Всё это остаётся внутренним анализом. Пользователь получает только оценку и рекомендацию."""
...
- range_from и range_to оба ВКЛЮЧИТЕЛЬНО (например, диапазон 0–9 означает range_from=0, range_to=9)
- Диапазоны не должны перекрываться и должны покрывать все возможные суммы баллов
...
async def generate_compatibility(
    ...
    try:
        ...
        if req.type == 'friendly':
            prompt = f"""Ты — система сравнения психологических профилей для дружеских отношений. Не используй слова «гармония», «идеально», «замечательно», «прекрасно», «удивительно». Не давай советов в духе «вам стоит», «попробуйте». Только констатация фактов на основе данных.

На вход подаются полные результаты тестов двух пользователей (А и Б) по пяти категориям: темперамент, личность (акцентуации), самооценка, общительность (включая дружелюбие и доминирование), профориентация.

Твоя задача: на основе сравнения профилей выдать структурированный ответ из четырёх обязательных блоков. Каждый блок — не более 3–4 предложений, строго по делу, без лишних оценок.

При формировании блоков учитывай следующие аспекты (но не обязательно перечислять все, только те, для которых есть чёткие данные в профилях):

1. В чем вы друг друга дополняете — оцените, как каждый расширяет границы другого:
   - Интеллектуальный диссонанс (логическое vs творческое мышление, дают друг другу непривычную оптику).
   - Эмоциональные роли (один — «жилетка» (эмпат), другой — «адвокат» (даёт советы и решения); важно, могут ли они меняться ролями).
   - Социальные связи (непересекающиеся круги общения расширяют мир, но без требования дружить с ними).
   - Уровень активности (один заводит на активности, другой учит наслаждаться паузой).
   Формулируй как факт: «А расширяет Б в [категория/черта], Б расширяет А в [категория/черта]».

2. К каким несовместимостям нужно быть терпеливыми — выделите зоны, где различия приемлемы без попыток «исправить»:
   - Разная частота встреч и инициативность (один пишет часто, другой редко, но при встрече тепло).
   - Разные политические / религиозные взгляды (если нет агрессивных попыток переубедить).
   - Несовпадение в хобби и вкусах (не обязаны любить одно и то же).
   - Разный уровень открытости (один любит глубокие обсуждения, другой — поверхностные новости).
   Укажите, в каких ситуациях это может проявляться, и завершите фразой: «Эти различия требуют терпения, попытки переделать друга приведут к дистанции».

3. Возможные общие цели — определите ситуативные цели, которые могут объединять:
   - Взаимопомощь в сложных периодах (карьерный кризис, разрыв, болезнь) — готовность поддержать.
   - Совместные проекты (походы, путешествия, волонтёрство, бизнес-идеи) — то, что делаете вместе сейчас.
   - Личностный рост друг друга — поддержка выборов друга, даже если они неудобны.
   - Сохранение связи во времени — осознанное намерение не терять контакт при переездах и смене статуса.
   Будьте конкретны: например, «поддержка друга в смене профессии», «совместная поездка в горы», «ежегодные встречи несмотря на расстояния».

4. На что обращать внимание для восстановления отношений — после обид или охлаждения:
   - Уважение к паузе (не давить, не выяснять отношения через силу).
   - Искренний юмор и лёгкость (возврат к совместным шуткам и приятным воспоминаниям без драмы).
   - Отсутствие обид за «неидеальность» (принятие, что друг может забыть о дате, если в остальном надёжен).
   - Честность о своих границах (мягкий, но чёткий разговор, если друг перешёл черту).
   - Создание нового совместного опыта (спонтанное новое дело, чтобы «записать» поверх ссоры новый позитивный след).
   Укажите, что важно для каждого из друзей в отдельности.

Все выводы должны быть сухими, фактологическими, без оценки «лучше/хуже».

Профиль пользователя А:
{my_portrait.content}

Профиль пользователя Б:
{friend_portrait.content}

Используй красивое форматирование Markdown (заголовки, списки). Не используй никаких вступлений, сразу выдавай результат анализа. Сгенерируй ответ строго по четырём блокам."""
        else:
            prompt = f"""Ты — система сравнения психологических профилей для партнёрских отношений. Не используй слова «гармония», «идеально», «замечательно», «прекрасно», «удивительно». Не давай советов в духе «вам стоит», «попробуйте». Только констатация фактов на основе данных.

Пол пользователя А: {req.my_gender}
Пол пользователя Б: {req.friend_gender}
...
1. В чем вы друг друга дополняете — оцените глубинные компенсации:
   - Эмоциональный баланс (кто паникует, кто сохраняет хладнокровие; кто помогает чувствовать живые эмоции, кто — не «зависать» в рацио).
   - Бытовая и финансовая компетенция (генератор идей / рисковый vs систематизатор / хранитель бюджета).
   - Социальная энергия (интроверт замедляет и рефлексирует, экстраверт вытаскивает в мир).
   - Сексуальный драйв (спонтанность и новизна vs глубина и эмоциональная связь).
   Формулируй как факт: «А компенсирует Б в [категория/черта], Б компенсирует А в [категория/черта]».

2. К каким несовместимостям нужно быть терпеливыми — выделите зоны, где различия могут вызывать трения, но их можно принимать без попыток переделать:
   - Разный уровень аккуратности / порядка (если не антисанитария).
   - Разная потребность в тактильности и личном пространстве.
   - Разный темп принятия решений.
   - Несовпадение либидо в отдельные фазы жизни (гормоны, стресс, возраст).
   Укажите конкретные ситуации вероятных конфликтов и завершите фразой: «Эти различия требуют терпения, попытки переделать партнёра приведут к напряжению».

3. Возможные общие цели — определите долгосрочные векторы, которые могут объединять:
   - Финансовая стратегия на 5+ лет (ипотека, инвестиции, уровень жизни, отношение к кредитам).
   - Модель семьи (дети, распределение родительства, отношение к вмешательству родственников).
   - География жизни (готовность к переездам ради карьеры).
   - Качество досуга в долгосроке (активная или спокойная старость, город или загород).
   Будьте конкретны, например: «совместное накопление на квартиру», «воспитание детей с акцентом на творчество», «переезд в другой город через 3 года».

4. На что обращать внимание для восстановления отношений — на основе темперамента и копинг-стратегий укажите механизмы перезагрузки:
   - Ритуал «точки сборки» (ежедневные 15 минут без гаджетов для обсуждения дня без претензий).
   - Скорость остывания и предпочтительный канал коммуникации после ссоры (письменная или устная).
   - Физический контакт как якорь (объятия, прикосновения).
   - Честность о своих «слепых зонах» (просьба дать время на остывание).
   - Совместное планирование приятного после примирения.
   Укажите, что важно для каждого из партнёров в отдельности.
