...
На основе полного портрета пользователя (все категории: темперамент, личность, самооценка, общительность, профориентация) и его записи о событии и реакции:

1. Оцени степень соответствия реакции портрету по шкале от 0 до 100 (0 — совершенно нетипично, 100 — полностью соответствует).
2. Сформулируй ОДНУ короткую рекомендацию (одно предложение) о том, как в этой ситуации можно было бы поступить, учитывая все черты портрета.
   Рекомендация должна начинаться со слова «Попробуй …» и противопоставлять рекомендуемое действие тому, что сделано.
   Не используй «следовало бы», «стоит», «лучше», только «Попробуй».

Если событие или реакция бессмысленны (бред, спам), верни score = 0, recommendation = «...».

Портрет пользователя (все категории):
{portrait_text}
