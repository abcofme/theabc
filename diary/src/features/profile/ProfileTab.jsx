...
export default function ProfileTab() {
  ...
  return (
    <div className="flex flex-col h-full relative select-none bg-transparent">
      ...
      <div className="flex items-center gap-4 p-4 sm:p-6 bg-neutral-900/60 border border-neutral-800/80 rounded-3xl mx-2 mt-2 mb-6 backdrop-blur-sm shadow-sm">
        {tgUser.photo_url ? (
          <img 
            src={tgUser.photo_url} 
            alt="Avatar" 
            className="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover shadow-lg border-2 border-neutral-700" 
          />
        ) : (
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-6 space-y-3">
        <h3 className="text-sm font-bold text-neutral-500 uppercase tracking-widest mb-3 ml-2">Все тесты</h3>
        ...
        {loading ? (
          ...
          <div className="text-center text-neutral-500 py-8 bg-neutral-900/40 rounded-3xl border border-neutral-800/80">
            Нет доступных тестов
          </div>
        ) : (
          categories.map(cat => (
            <div key={cat.id} className="bg-neutral-900/70 border border-neutral-800 rounded-2xl overflow-hidden transition-all duration-200">
                    ...
                    cat.tests.map(test => (
                      <div 
                        ...
                        className={`flex items-center justify-between py-3.5 border-b border-neutral-800/50 last:border-0 ${
                          test.passed 
                            ? 'cursor-pointer hover:bg-neutral-800/40 -mx-4 px-4 transition-colors active:bg-neutral-800' 
      ...
      {/* 3. ВСПЛЫВАЮЩЕЕ ОКНО С РЕЗУЛЬТАТОМ ТЕСТА */}
      {selectedResult && (
          ...
          <div className="relative bg-neutral-900 border border-neutral-700 rounded-[2rem] w-full max-w-lg shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 sm:p-6 border-b border-neutral-800 flex justify-between items-start">
              ...
              <button 
                ...
                className="p-2 bg-neutral-800/80 hover:bg-neutral-700 rounded-full text-neutral-400 transition-colors shrink-0"
              >
                <X size={20} />
              </button>
            </div>
            ...
            {/* Текст результата */}
            <div className="p-5 sm:p-6 overflow-y-auto text-neutral-300 text-sm sm:text-base leading-relaxed whitespace-pre-wrap scrollbar-hide">
              {selectedResult.result_text || "К сожалению, текстовый результат для этого теста не найден."}
            </div>

            {/* Кнопка закрытия снизу */}
            <div className="p-4 sm:p-5 border-t border-neutral-800">
              <button 
                onClick={() => setSelectedResult(null)}
                className="w-full py-4 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white rounded-2xl font-semibold text-lg transition-colors shadow-lg shadow-blue-900/20"
              >
                Понятно
              </button>
            </div>
